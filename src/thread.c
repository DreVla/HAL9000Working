    ...
    )
{
    ...
    {
        InsertOrderedList(&m_threadSystemData.ReadyThreadsList, &pThread->ReadyList, _PriorityFunction, NULL);
    }
    if (!bForcedYield)
    {
    ...
    )
{
    ...
    LockAcquire(&m_threadSystemData.ReadyThreadsLock, &dummyState);
    InsertOrderedList(&m_threadSystemData.ReadyThreadsList, &Thread->ReadyList, _PriorityFunction, NULL);
    ...
    )
{
    ...
    __try
    {
        ...
        InsertOrderedList(&m_threadSystemData.AllThreadsList, &pThread->AllList, _PriorityFunction, NULL);
...
_PriorityFunction(
	...
	//cast to thread
	PTHREAD A = CONTAINING_RECORD(a, THREAD, ReadyList);
	PTHREAD B = CONTAINING_RECORD(b, THREAD, ReadyList);
